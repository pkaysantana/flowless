  ...
  function render(route) {
    ...
    var labelAction = collected
      ? el(
          ...
          // NO target="_blank". A new tab is a fresh page load, which re-seeds
          // the in-memory store with new random tokens — so this token would
          // resolve to nothing and the label would come up empty. Same-tab hash
          // navigation is what keeps the store alive across the walkthrough.
          el("a", {
        ...
        ui.panel("Take these samples", null, [
          ...
          el(
            ...
            request.tests.map(function (test) {
              var panel = FL.data.testPanelByCode(test.code);
              return el(
                "li",
                { class: "specimen" },
