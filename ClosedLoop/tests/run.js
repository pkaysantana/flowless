 ...
 *
 * The domain layer is plain browser JavaScript attached to a CR namespace, so
...
describe("store — the whole relay, end to end", function () {
  ...
  it("finds a request by its token, however it is typed", function () {
    ...
    assert(store.findByToken("  " + request.token + " "));
    assert(store.findByToken("https://care-relay.demo/#/lab/" + request.token));
    assert(!store.findByToken("CR-ZZZZ-ZZZZ"));
  });
...
describe("encoders", function () {
  it("round-trips a QR payload through the module layout", function () {
    var qr = CR.qr.encode("https://care-relay.demo/#/present/CR-7K3D-9QW2");
  ...
  it("encodes every token character in Code 39", function () {
    for (var i = 0; i < 30; i++) {
      assert(CR.code39.isEncodable(d.newToken()), "tokens must fit on a specimen label");
    }
  });

  it("refuses characters Code 39 cannot carry", function () {
    assert(!CR.code39.isEncodable("lower case ok?"));
    assert(!CR.code39.isEncodable(""));
  });
});
